...
public:
    ...
    void preProcess() {
        ...
        long i,last, arc_num, arc_new_num,tail_node_id,up_bound;
        ...
        long long int cost,cap_out,cap_in;
        PT(cost);
...
restart:
    ...
    void mcmf( double *objective_cost) {
        int cc = 0;
    ...
    int addServerAndDeployPrice(double *cost){
        ...
        while(srcArc != (srcNode+1)->getSuspended()) {
            if((capArrayYCWVar[ srcArc == NULL ? -1 : srcArc - arcsArrayYCWVar ] - srcArc->getCAP()) > 0) {
                ...
                total += serverOutput;
                if(serverOutput > serverMaxOutPutLimit[serverNode-nodesArrayYCWVar]){
                    serverOutput = serverMaxOutPutLimit[serverNode-nodesArrayYCWVar];
                }
